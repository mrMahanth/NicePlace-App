...
class PostProjectNearbyPlacesScreen extends StatefulWidget {
  final int projectId;

  const PostProjectNearbyPlacesScreen({super.key, required this.projectId});

  @override
  State<PostProjectNearbyPlacesScreen> createState() =>
      _PostProjectNearbyPlacesScreenState();
}

class _PostProjectNearbyPlacesScreenState
    extends State<PostProjectNearbyPlacesScreen> {
  ...
  Future<void> _loadData() async {
    ...
    try {
      ...
      final places = (project['nearby_places'] as List<dynamic>? ?? [])
              ...
              actions: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    final result = await ProjectService.addNearbyPlace(
  ...
  Future<void> _deletePlace(NearbyPlaceModel place) async {
    final result = await ProjectService.deleteNearbyPlace(
      projectId: widget.projectId,
      placeId: place.id,
    );
  ...
  void _continue() {
    Navigator.push(
      context,
      MaterialPageRoute(
        builder: (_) => PostProjectBulkUnitsScreen(projectId: widget.projectId),
      ),
    );
  }
